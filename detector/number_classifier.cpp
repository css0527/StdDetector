...
: threshold(thre), ignore_classes_(ignore_classes)
{
  ...
  std::cout << "NumberClassifier loaded with " << class_names_.size() << " classes: ";
...
cv::Mat NumberClassifier::extractNumber(const cv::Mat & src, const Armor & armor)
{
  ...
  // 在 number_classifier.cpp 中，将 ArmorType::big 改为 ArmorType::LARGE
  const int warp_width = (armor.type == ArmorType::LARGE) ? large_armor_width : small_armor_width;
...
void NumberClassifier::classify(Armor & armor)
{
  ...
  if (label_id >= 0 && label_id < static_cast<int>(class_names_.size())) {
    ...
    // 映射类名到 ArmorName
    if (class_name == "1" || class_name == "one")
      armor.name = ArmorName::one;
    else if (class_name == "2" || class_name == "two")
      armor.name = ArmorName::two;
    else if (class_name == "3" || class_name == "three")
      armor.name = ArmorName::three;
    else if (class_name == "4" || class_name == "four")
      armor.name = ArmorName::four;
    else if (class_name == "5" || class_name == "five")
      armor.name = ArmorName::five;
    else if (class_name == "sentry")
      armor.name = ArmorName::sentry;
    else if (class_name == "outpost")
      armor.name = ArmorName::outpost;
    else if (class_name == "base")
...
void NumberClassifier::eraseIgnoreClasses(std::list<Armor> & armors)
{
  armors.remove_if([this](const Armor & armor) {
    ...
    if (armor.type == ArmorType::LARGE) {
      mismatch =
        (armor.name == ArmorName::outpost || armor.name == ArmorName::two ||
         armor.name == ArmorName::sentry);
    } else if (armor.type == ArmorType::SMALL) {
      mismatch = (armor.name == ArmorName::one || armor.name == ArmorName::base);
    }
    return mismatch;
  });
}
